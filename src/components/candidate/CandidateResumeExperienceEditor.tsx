...
import { Building, Clock, Sparkles, Trash2, Pencil } from 'lucide-react';
...
import { ResumeFieldLabel, ResumeGhostButton, ResumePrimaryButton, ResumeSectionCard, resumeInputClass, resumeTextareaClass } from './CandidateResumeEditorPrimitives';
      ...
      <ResumeSectionCard className="!border-0 !bg-white !p-0">
        <form className="space-y-4" onSubmit={(event) => { event.preventDefault(); onSaveExperience(); }}>
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            <div><ResumeFieldLabel>Data de início</ResumeFieldLabel><input type="date" value={tempExperience?.startDate || ''} onChange={(event) => onTempExperienceChange((prev) => { if (!prev) return null; const startDate = event.target.value; const endDate = prev.endDate && prev.endDate < startDate ? '' : prev.endDate; return { ...prev, startDate, endDate }; })} required className={resumeInputClass} /></div>
            <div><ResumeFieldLabel>Data de término</ResumeFieldLabel><input type="date" value={tempExperience?.endDate || ''} min={tempExperience?.startDate || undefined} onChange={(event) => onTempExperienceChange((prev) => { if (!prev) return null; const endDate = event.target.value; if (prev.startDate && endDate && endDate < prev.startDate) return { ...prev, endDate: '' }; return { ...prev, endDate }; })} disabled={tempExperience?.current} className={resumeInputClass} /></div>
          ...
          <div className="flex justify-end gap-2 pt-2"><ResumeGhostButton onClick={onCancelModal}>Cancelar</ResumeGhostButton><ResumePrimaryButton type="submit" className="shadow-none">Adicionar</ResumePrimaryButton></div>
        </form>
      </ResumeSectionCard>
    );
  }
  ...
  return (
    <div className="space-y-4">
      {isFirstJob ? (
        ...
        <div className="px-4 py-8 text-center"><p className="text-[13px] font-semibold text-[#343241]">Nenhuma experiência adicionada</p><p className="mx-auto mt-1 max-w-xs text-[12px] font-medium text-slate-400">Inclua seus trabalhos anteriores para fortalecer seu currículo.</p></div>
      ) : (
        <div className="space-y-3">
          {experiences.map((experience) => (
            <ResumeSectionCard key={experience.id} className="!border-[#940dff]/12 shadow-[0_8px_18px_rgba(148,13,255,0.055)]">
              <div className="flex items-start justify-between gap-3">
                <div className="flex min-w-0 gap-3">
                  <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[#f3e5ff] text-[#940dff]">
                    <Building size={17} />
                  </div>
                  <div className="min-w-0">
                  <h3 className="text-[14px] font-semibold text-[#343241]">{experience.role}</h3>
                  <p className="mt-1 text-[12px] font-semibold text-slate-500">{experience.company}</p>
                  <p className="mt-1 flex items-center gap-1.5 text-[11px] font-medium text-slate-400">
                    <Clock size={12} />
                    {formatExperienceDurationWithPeriod(experience.startDate, experience.endDate, experience.current) || calculateDuration(experience.startDate, experience.endDate, experience.current)}
                  </p>
                  </div>
                </div>
