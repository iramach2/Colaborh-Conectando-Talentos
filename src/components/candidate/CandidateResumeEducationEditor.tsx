        ...
        educations.map((education) => (
          <ResumeSectionCard key={education.id} className="!border-[#940dff]/12 shadow-[0_8px_18px_rgba(148,13,255,0.055)]">
            <div className="flex items-start justify-between gap-3">
              ...
              <div className="flex shrink-0 gap-2"><button type="button" onClick={() => onEditEducation(education)} className="flex h-8 w-8 items-center justify-center rounded-xl border border-[#ffc24b] bg-[#ffc24b] text-white" aria-label="Editar formação"><Pencil size={14} /></button><button type="button" onClick={() => onRemoveEducation(education.id)} className="flex h-8 w-8 items-center justify-center rounded-xl border border-[#ff4b8c] bg-[#ff4b8c] text-white" aria-label="Remover formação"><Trash2 size={14} /></button></div>
            </div>
          </ResumeSectionCard>
        ))
      )}
