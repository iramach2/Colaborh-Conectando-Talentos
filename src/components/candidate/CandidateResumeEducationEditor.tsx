    ...
    <div className="space-y-3">
      {educations.length === 0 ? (
        ...
        educations.map((education) => (
          <ResumeSectionCard key={education.id} className="!border-[#940dff]/12 shadow-[0_8px_18px_rgba(148,13,255,0.055)]">
            <div className="flex items-start justify-between gap-3">
              <div className="flex min-w-0 gap-3"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[#f3e5ff] text-[#940dff]"><GraduationCap size={17} /></div><div className="min-w-0"><h3 className="text-[14px] font-semibold text-[#343241]">{education.course}</h3><p className="mt-1 text-[12px] font-semibold text-slate-500">{education.institution}</p><div className="mt-2 flex flex-wrap items-center gap-2"><span className={`rounded-lg px-2 py-1 text-[10px] font-semibold ${education.status === 'Completo' ? 'bg-[#f3e5ff] text-[#940dff]' : education.status === 'Cursando' ? 'bg-[#f3e5ff] text-[#940dff]' : 'bg-slate-100 text-slate-500'}`}>{education.status}</span><span className="text-[11px] font-medium text-slate-400">Ano: {education.gradYear}</span></div></div></div>
              <div className="flex shrink-0 gap-2"><button type="button" onClick={() => onEditEducation(education)} className="flex h-8 w-8 items-center justify-center rounded-full border border-[#ffc24b] bg-[#ffc24b] text-white" aria-label="Editar formação"><Pencil size={14} /></button><button type="button" onClick={() => onRemoveEducation(education.id)} className="flex h-8 w-8 items-center justify-center rounded-full border border-[#ff4b8c] bg-[#ff4b8c] text-white" aria-label="Remover formação"><Trash2 size={14} /></button></div>
            </div>
          </ResumeSectionCard>
        ))
      )}
